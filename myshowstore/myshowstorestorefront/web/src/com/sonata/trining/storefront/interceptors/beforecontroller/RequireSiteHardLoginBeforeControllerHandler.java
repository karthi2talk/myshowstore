...
public class RequireSiteHardLoginBeforeControllerHandler implements BeforeControllerHandler
{
	...
	private String loginControllerMethod;

	public String getLoginControllerMethod()
	{
		return loginControllerMethod;
	}

	@Required
	public void setLoginControllerMethod(final String loginControllerMethod)
	{
		this.loginControllerMethod = loginControllerMethod;
	}
			...
			final HandlerMethod handler) throws Exception
	{
		...
		if (request.isSecure())
		{
			...
			if (isAnonymousUser && !getLoginControllerMethod().equals(handler.getMethod().getName()))
			{
